// v0 技术验证入口:
//   npm start    —— 手动模式,窗口常驻供肉眼检查
//   npm run spikes —— 自测模式:跑完四个 spike,写 spike-results.json 后自动退出
// Spike A:毛玻璃材质解析与降级链(面板窗)
// Spike B:透明置顶窗 + 帧动画 + FPS(宠物窗)
// Spike C:Claude Agent SDK 在 Electron main 内运行
// Spike D:宠物窗分区点击穿透(透明区穿透、精灵可点)
...
function createPanelWindow(effect) {
  ...
  win.loadFile(join(__dirname, "..", "panel", "index.html"), { search: query.toString() });
  return win;
}
...
function createPetWindow() {
  ...
  const win = new BrowserWindow({
    ...
    webPreferences: {
      ...
      preload: join(__dirname, "..", "pet", "preload.js"),
    },
  });
  win.loadFile(join(__dirname, "..", "pet", "index.html"));
  return win;
}

function bindPetIpc(win) {
  // Spike D:渲染层按指针是否悬停精灵上报,主进程切换点击穿透
...
async function spikeClaudeSdk() {
  ...
  try {
    ...
    entry.exports = Object.keys(mod).slice(0, 12);
    ...
    else if (/auth|api key|credit|login|oauth/i.test(text)) entry.outcome = "auth-required";
...
app.whenReady().then(async () => {
  ...
  const panel = createPanelWindow(effect);
  ...
  if (SELF_TEST) {
    ...
    console.log("[v0] 手动模式:关闭窗口即退出。Spike C 触发:SPIKE_RUN_SDK=1 npm start");
  }
});
...
process.on("uncaughtException", (error) => {
  ...
  writeFileSync(join(app.getAppPath(), "spike-results.json"), JSON.stringify(results, null, 2));
