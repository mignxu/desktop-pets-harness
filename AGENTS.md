...
3. 当前阶段(2026-08-30):**第一条真 turn 已跑通**——真实 Claude Code adapter(`minimax-m2.5` @ 用户 new-api 中转,配置在 `store/api-config.json`)+ 模拟审批闭环全部通过;面板 AionUi 复刻、宠物窗 act_conf Player 均已实测。下一步聚焦真实工具任务与契约固化

## 下一步(按优先级,接手后从这里开始)

1. **真实审批闭环实测**:面板发一个会触发工具的任务(如"看看 nest 目录里有什么")→ 验证 canUseTool → 审批卡 + 宠物"被吵醒"+ 气泡召唤 → 面板决定 → 任务继续(模拟已全通,真适配器走同一契约)
2. **契约固化**:用真 turn 的真实事件形状校验契约后,固化为 zod schema + 每 adapter 显式 capabilities(能力表驱动 UI 降级)——放在接第二家 harness 之前
3. 之后按笔记 7.12 待定清单:第二家 harness(ACP 兜底 vs 原生)、Win11 补验 acrylic、多工作区、侧栏图标换正式图标集、会话管理(重命名/删除)
