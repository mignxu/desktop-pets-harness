...
3. 当前阶段(2026-09-01 更新):**v1 全功能完成 + 面板已迁移 React18+Arco**——真 turn(minimax-m2.5 @ 用户 new-api)跑通、markdown/diff/持久化/审批闭环(模拟+真实 canUseTool 两次实操)全通过;**打包(electron-builder)已打通(2026-09-01)**:winCodeSign 缓存预填 + 清代理 + ELECTRON_MIRROR 加速后,`--dir` 免装验证构建成功(dist/win-unpacked 产出);面板产物由 `build/panel` 改名 `panel-dist`,避开 electron-builder 对 build/ 的自动 `!build` 剪枝(否则面板打不进包、运行后面板空白)

## 下一步(按优先级,接手后从这里开始)

1. **打包收尾(已闭环)**:`--dir` 免装验证构建通过(dist/win-unpacked 产出,面板 panel-dist 已进包);下一步可选 `npm run dist` 产出 nsis + portable 安装包(exe 改写因无签名 `-c.win.signAndEditExecutable=false` 跳过,可接受)。图标脚本 `npm run icon` 仍崩(gen-icon.js 调 `app.whenReady` 报错),但 build/icon.png 已存在不阻塞;待回头修
2. **审批闭环验证(C1)**:无头集成测试已通过(`node scripts/test-approval-loop.cjs`,15 项断言全 PASS)——契约事件流 + 状态机(working→waitingInteraction→idle)+ 宠物待审批广播(被吵醒+气泡)+ 能力表驱动按钮 + 全链路契约零误报,均经 MockSession 驱动验证;**剩余真实 GUI re-test**(真 canUseTool + 本机凭据)留待你本机实测
...
npm run panel:build  # 面板渲染层构建(改 src/panel-app 后必须执行,Electron 加载 panel-dist)
...
- 目录:`src/main`(入口分发:v1-main 产品 / spike-main 验证)、`src/shared`(契约)、`src/adapter`(harness 适配:claude-code / mock)、`src/host`(线程管理与聚合)、`src/pet`(宠物窗 Player)、`src/panel`(preload)、`src/panel-app`(面板 React 源码,构建到 `panel-dist`)、`src/spike`(v0 遗迹)、`tools/`(素材脚本)、`demo/`(预览 scratch,不进主线)、`小呆/` `像素猫meme/`(宠物素材包)
